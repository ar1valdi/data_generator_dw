...
def generate_batch(date_range, file_prefix, prev_prefix):
    ...
    write_to_csv(faculties, "Katedry", file_prefix)
    ...
    write_to_csv(workers, "Pracownicy_CSV", file_prefix)
    write_to_csv(workers_sql, "Pracownicy", file_prefix)
    ...
    write_to_csv(courses, "Kursy", file_prefix)
    write_to_csv(studies, "Kierunki", file_prefix)
    ...
    write_to_csv(students_sql, "Studenci", file_prefix)
    write_to_csv(students, "Studenci_CSV", file_prefix)

    # PARTICIPATIONS IN COURSES
    if not prev_prefix:
        participations = generate_all_participations(courses, students_sql, students)
        write_to_csv(participations, "UdzialyWKursach", file_prefix)
        ...
        if s is None:
            return
        write_to_csv([s], "Studenci_CSV", file_prefix)
